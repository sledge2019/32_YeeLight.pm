...
YeeLight_Open($;$)
{
    ...
    $reopen = 0 if !defined $reopen;
    DevIo_OpenDev($hash, $reopen, "YeeLight_ConnInit", "YeeLight_ConnError");
}
...
YeeLight_Notify
{
    ...
    {
        if ($devName eq "global" && $event =~ /^ATTR $ownName keepAlive/ )
        {
...
YeeLight_GetUpdate
{
    ...
    YeeLight_StartUpdateTimer($hash);
...
YeeLight_ParseStatusRequest
{
    ...
    if ($answer)
    {
        ...
        Log3 $name, 4, "$name full statusrequest";
    }
...
YeeLight_Cleanup(@) {
  my ($hash) = @_;
  YeeLight_RemoveTimerCheck($hash);
  DevIo_CloseDev($hash);
}
...
YeeLight_ConnInit($)
{
    my ($hash) = @_;
    my $name = $hash->{NAME};
    Log3 $name, 3, "YeeLight $name connection established";
...
sub 
YeeLight_RemoveTimerCheck {
    my ($hash) = @_;
    RemoveInternalTimer( $hash, "YeeLight_GetUpdate" );
}
...
YeeLight_Ready
{
    ...
    YeeLight_Open($hash, 1) if ( $hash->{STATE} eq "disconnected" ) && AttrVal( $name, "autoConnect", 1 );
}
